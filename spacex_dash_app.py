...
def update_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(spacex_df, names='Launch Site', title='Total Success Launches by Site', color='class', 
                     color_discrete_map={1: 'green', 0: 'red'}, labels={'class': 'Launch Outcome'})
        fig.update_traces(textinfo='label+percent', pull=[0.1] * len(spacex_df['Launch Site'].unique()))
    else:
